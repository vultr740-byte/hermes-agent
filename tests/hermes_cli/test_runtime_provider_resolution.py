...
def test_custom_endpoint_keeps_openai_env_key_when_entrypoint_persisted_base_url(monkeypatch):
    """Railway entrypoint persists OPENAI_BASE_URL into config.yaml.

    Runtime resolution must still treat OPENAI_API_KEY as the paired key when
    the persisted model.base_url equals the current OPENAI_BASE_URL. Otherwise
    custom OpenAI-compatible deployments send Bearer no-key-required and 401.
    """
    ...
    monkeypatch.delenv("OPENROUTER_BASE_URL", raising=False)
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    monkeypatch.setenv("OPENAI_BASE_URL", "https://api.openai-proxy.example/v1")
    monkeypatch.setenv("OPENAI_API_KEY", "proxy-key")

    resolved = rp.resolve_runtime_provider(requested="custom")

    assert resolved["provider"] == "custom"
    assert resolved["base_url"] == "https://api.openai-proxy.example/v1"
