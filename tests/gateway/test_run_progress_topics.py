...
class EmptyFinalWithImageToolResultAgent:
    def __init__(self, **kwargs):
        self.tools = []
        ...
        return {
            ...
            "messages": [
                ...
                },
                {
                    ...
                    "content": (
                        '{"success": true, "image": "/tmp/fish.png", '
                        '"media_tag": "MEDIA:/tmp/fish.png"}'
                    ),
                },
            ],
            "api_calls": 1,
...
@pytest.mark.asyncio
async def test_run_agent_recovers_media_from_tool_results_when_final_response_empty(
    ...
    fake_run_agent = types.ModuleType("run_agent")
    fake_run_agent.AIAgent = EmptyFinalWithImageToolResultAgent
    monkeypatch.setitem(sys.modules, "run_agent", fake_run_agent)
    ...
    assert result["final_response"] == "MEDIA:/tmp/fish.png"
...
def test_gateway_billing_failure_checks_final_response_after_tool_error(monkeypatch):
    gateway_run = importlib.import_module("gateway.run")
    monkeypatch.setenv("RECHARGE_TARGET", "weixin_demo_target")

    result = gateway_run._gateway_billing_failure_messages(
        {
            "failed": True,
...
def test_gateway_billing_failure_uses_structured_reason_as_fallback(monkeypatch):
    gateway_run = importlib.import_module("gateway.run")
    monkeypatch.setenv("RECHARGE_TARGET", "weixin_demo_target")
    ...
    result = gateway_run._gateway_billing_failure_messages(
        {
            ...
            "final_response": "The model failed after a tool error.",
        }
    )
