...
<div class="grid-3">
  <div class="pillar">
    ...
    <div class="d">PLAYBOOK.md<br>+ 项目知识库<br>+ 护栏文件</div>
  </div>
  <!-- ... 更多 pillar ... -->
</div>
```
...
`assets/template.html` 底部的 module script 会尝试加载 **jsDelivr CDN** 的 Motion One。加载失败时会强制把所有带 `data-anim` 的元素设为 `opacity:1`——内容永远可读,只是没有入场动效。
...
- **内容静态时也不显示?** 打开浏览器控制台看 Motion One 加载错误;模板应该自动 reveal 所有内容
