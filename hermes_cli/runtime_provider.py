...
def _resolve_custom_runtime(
    *,
    ...
    requested_norm = (requested_provider or "").strip().lower()
    original_requested_norm = requested_norm
    if requested_norm and requested_norm != "custom":
        try:
            from hermes_cli.auth import resolve_provider as _resolve_provider
    ...
    if cfg_base_url.strip() and not explicit_base_url:
        if requested_norm == "auto":
            if not cfg_provider or cfg_provider == "auto" or _config_base_url_trustworthy_for_bare_custom(
                cfg_base_url, cfg_provider
            ):
                use_config_base_url = True
        elif requested_norm == "custom" and _config_base_url_trustworthy_for_bare_custom(
    ...
    elif env_custom_base_url:
        base_url_source = "custom_env"
    elif use_config_base_url and cfg_base_url.strip():
        base_url_source = "config"
    elif env_openai_base_url:
        base_url_source = "openai_env"
    ...
    if base_url:
        pool_result = _try_resolve_from_custom_pool(
            ...
            provider_name=(
                original_requested_norm
                if original_requested_norm and original_requested_norm != "custom"
                else None
            ),
        )
        if pool_result:
    ...
    _is_openai_azure = base_url_host_matches(base_url, "openai.azure.com")
    _is_openrouter_url = base_url_host_matches(base_url, "openrouter.ai")
    # Docker/Railway startup writes OPENAI_BASE_URL into config.yaml so the
    # endpoint survives restarts. After that write, the selected base_url looks
    # config-sourced even though OPENAI_API_KEY is still the paired env secret.
    # Preserve that explicit env pairing without reopening the broad
    # "send OPENAI_API_KEY to any custom config endpoint" leak fixed in #28660.
    env_openai_base_url_matches = bool(
        env_openai_base_url
        and base_url.rstrip("/") == env_openai_base_url.rstrip("/")
    )
    api_key_candidates = [
        (explicit_api_key or "").strip(),
        (cfg_api_key if use_config_base_url else ""),
        ...
        (
            ...
            if (
                _is_openai_url
                or _is_openai_azure
                or base_url_source == "openai_env"
                or env_openai_base_url_matches
            )
            else ""
        ),
